...
public class TicketRepositoryImpl extends NamedParameterJdbcDaoSupport implements TicketRepository{
    ...
    private final String queryAllMovies;
    ...
    public TicketRepositoryImpl(String queryAllVenues, String queryFindVenueByLevel,
        ...
        this.queryAllMovies=queryAllMovies;
    }

    
    public boolean addMoviesdata(MovieData movieData)
    {
    	boolean response=true;    	
        /*String sqlQuery="insert into Movie_Data(id,MOVIE_NAME,MOVIE_ACTOR,MOVIE_STAR) values("+movieData.getMovieId()+movieData.getMovieName()+movieData.getMovieActor()+movieData.getMovieStar()+")";
        
        try{
        	 getJdbcTemplate().execute(sqlQuery);
        	 response=true;
            LOGGER.debug("Finished adding movie", response);
        }catch (DataAccessException e){
        	 response=false;
            throw new TicketException("Unable to add movie with input " + movieData.getMovieId(), e);
        }*/
    	return response;
    }
    
    public List<MovieData> showAllmoviesList()
    {
    	List<MovieData> movielist= getJdbcTemplate().queryForList(queryAllMovies,null,null, new MovieRowMapper());
    	return movielist;
    }
